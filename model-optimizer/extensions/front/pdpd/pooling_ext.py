...
class PoolingFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        pdpd_op = node.pb['op']
        pdpd_attr = pdpd_op.all_attrs()
        pdpd_block = node.pb['block']
        global_pooling = False
        pooling_convention = 'valid'
        if pdpd_attr['global_pooling'] or (pdpd_attr['adaptive'] and pdpd_attr['ksize'] == [1,1]):
            method = pdpd_attr['pooling_type']
            global_pooling = True
            pooling_convention = 'full'
        input_shape = kernel_shape = pdpd_block.var(pdpd_op.input("X")[0]).shape
        k_size = pdpd_attr['ksize']
        paddings = pdpd_attr['paddings']
        ...
        final_kernel_shape = np.array([1, 1, *[x for x in k_size]], dtype=np.int64) if k_size is not None else None

        pads = np.array([pdpd_attr['paddings'], pdpd_attr['paddings']], dtype=np.int64)
        final_pads = np.array([[0, 0], [0, 0], *pads], dtype=np.int64) if pads is not None else None

        # Extract strides attribute
        # In case if strides is not specified it will be set in default (1) in infer function
        strides = np.int64(pdpd_attr["strides"])
        final_strides = np.array([1, 1, *strides], dtype=np.int64) if strides is not None else None
        
        final_dilations = None
        if 'dilations' in pdpd_attr:
            dilations = np.int64(pdpd_attr["dilations"])
        ...
        auto_pad = pdpd_attr['padding_algorithm']
        if auto_pad == 'SAME':
            auto_pad = 'SAME_UPPER'
        elif auto_pad == 'VALID':
            auto_pad = 'VALID'
            ...
            auto_pad = None
        # TODO check if it is a correct choice for pdpd
        
        rt = 'ceil' if pdpd_attr['ceil_mode'] else 'floor'
        attrs = {
            'op': node.op,
            'auto_pad': auto_pad,
...
def common_onnx_pool_extractor(node):
    ...
    # hard-code ceil rounding & exculde_pad
    attrs = {
        'op': node.op,
        'auto_pad': auto_pad,
        'window': final_kernel_shape,
