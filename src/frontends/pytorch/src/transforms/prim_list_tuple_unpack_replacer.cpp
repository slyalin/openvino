...
bool DecomposeListTupleParameters::run_on_model(const std::shared_ptr<Model>& model) {
    ...
    while (!parameters.empty()) {
        ...
        for(auto consumer: consumers) {
            //auto tuple_unpack = cast_fw_node(input_node, "prim::TupleUnpack");
            auto node = consumer.get_node()->shared_from_this();
            auto tuple_unpack = std::dynamic_pointer_cast<ov::op::util::TupleUnpack>(node);
        ...
        for (size_t i = 0; i < num_outputs; ++i) {
            ...
            for (auto outputs: consumer_outputs) {
                ...
                OPENVINO_ASSERT(PartialShape::merge_into(ps, output.get_partial_shape()), "Consumers for upack op have incompatible shape");
                OPENVINO_ASSERT(element::Type::merge(et, et, output.get_element_type()), "Consumers have incompatible types");
