...
std::shared_ptr<Node> _get_im2col_indices_along_dim(NodeContext& context,
...
OutputVector translate_im2col(NodeContext& context) {
    ...
    auto blocks_row_indices =
        _get_im2col_indices_along_dim(context, input_h, kernel_h, dilation_h, padding_h, stride_h);
    auto blocks_col_indices =
        _get_im2col_indices_along_dim(context, input_w, kernel_w, dilation_w, padding_w, stride_w);
