...
ov::pass::StateManagementPattern::StateManagementPattern(ParameterVector& kv_parameters,
    ...
    auto kv_shaping = [=](const std::shared_ptr<Node>& kv_concat, std::shared_ptr<Node>& unsqueeze) {
        // Return unsqeeze (return param) to deduce number of kv heads in the place where they are being broadcases in case of GQA and MQ
    ...
    ov::matcher_pass_callback callback = [=,
        ...
        auto extract_num_kv_heads = [=, &pattern_map](std::shared_ptr<Node> unsqueeze) {
            ...
            if (pattern_map.find(unsqueeze) != pattern_map.end()) {
                ...
                unsqueeze = pattern_map.at(unsqueeze).get_node_shared_ptr(); //we reassign the pointer. Ok?
                ...
                if (first_element == 0 || first_element == -rank.get_length()) { // there should be at least one dimension to the left
                    // -rank.get_length() correct??
                    return ov::Dimension();
                }
