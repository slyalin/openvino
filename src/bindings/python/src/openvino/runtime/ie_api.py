...
def set_scalar_tensor(request: _InferRequestWrapper, tensor: Tensor, key: Union[str, int, ConstOutput] = None) -> None:
    if key is None:
...
def normalize_inputs(request: _InferRequestWrapper, inputs: dict) -> dict:
...
class InferRequest(_InferRequestWrapper):
    ...
    def postprocessed(self, inputs):
        return postprocess_results(super().infer(_data_dispatch(
            self,
            inputs,
            is_shared=shared_memory,
        )), super().model_outputs, self.model)
