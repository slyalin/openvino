...
class OPENVINO_API Any {
    ...
    public:
        ...
        virtual std::shared_ptr<Variant> init(const std::shared_ptr<Node>& node);
        virtual std::shared_ptr<Variant> merge(const std::vector<std::shared_ptr<Node>>& nodes);
    ...
    struct Impl<T, typename std::enable_if<std::is_convertible<T, std::shared_ptr<Variant>>::value>::type>
        ...
        std::shared_ptr<Variant> as_variant() const override {
            return std::static_pointer_cast<Variant>(variant);
        }
        bool is_copyable() const override {
            return static_cast<Variant*>(variant.get())->is_copyable();
        }
        std::shared_ptr<Variant> init(const std::shared_ptr<Node>& node) override {
            return static_cast<Variant*>(variant.get())->init(node);
        }
        std::shared_ptr<Variant> merge(const std::vector<std::shared_ptr<Node>>& nodes) override {
    ...
    typename std::enable_if<std::is_convertible<T, std::shared_ptr<Variant>>::value, T>::type&& as() && {
        if (_impl == nullptr) {
            ...
            if (_impl->type_info() == typeid(decay_t<T>)) {
                ...
                auto variant = _impl->as_variant();
                if (variant == nullptr) {
                    throw ov::Exception{"Any does not contains pointer to variant"};
                }
                _variant_impl =
                    std::make_shared<Impl<decay_t<T>>>(std::dynamic_pointer_cast<typename T::element_type>(variant));
                return _variant_impl->as<T>();
            }
        }
    }
    ...
    typename std::enable_if<std::is_convertible<T, std::shared_ptr<Variant>>::value, T>::type& as() & {
        if (_impl == nullptr) {
            ...
            if (_impl->type_info() == typeid(decay_t<T>)) {
                ...
                auto variant = _impl->as_variant();
                if (variant == nullptr) {
                    throw ov::Exception{"Any dose not contains pointer to variant"};
                }
                _variant_impl =
                    std::make_shared<Impl<decay_t<T>>>(std::dynamic_pointer_cast<typename T::element_type>(variant));
                return _variant_impl->as<T>();
