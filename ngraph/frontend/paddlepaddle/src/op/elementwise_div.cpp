...
OutputVector elementwise_div (const NodeContext& node) {
    ...
    if ((axis == -1) || (axis == x_shape.size() - 1) || (x_shape.size() == y_shape.size())) {
        return {std::make_shared<ngraph::opset6::Divide>(x, y)};
    }
    else {
        ...
        return {std::make_shared<ngraph::opset6::Divide>(x, y_node)};             
    }       
}
